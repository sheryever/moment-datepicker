...
!function ($, moment, undefined) {
    ...
    var Datepicker = function (element, options) {
        ...
        this.format = options.format || this.element.data('datepicker-format') || moment.localeData('ar').longDateFormat('L');
    ...
    Datepicker.prototype = {
        ...
        fillDow: function () {
            ...
            var html = '<tr>';
            var daysMin = $.proxy(moment.localeData('ar').weekdaysMin, moment.localeData('ar'));
        ...
        fillMonths: function () {
            var html = '';
            var i = 0
            var monthsShort = $.proxy(moment.localeData('ar').hMonthsShort, moment.localeData('ar'));
            while (i < 12) {
            	html += '<span class="month">' + monthsShort(moment().startOf('hmonth').hMonth(i++)) + '</span>';
            }
            this.picker.find('.datepicker-months td').append(html);
        },

        fill: function () {
            var year = this.viewDate.hYear();
            var month = this.viewDate.hMonth();
            var currentMoment = this.get();
            var currentDate = currentMoment ? currentMoment.valueOf() : null; //TODO: use diff
            var currentYear = currentMoment ? currentMoment.hYear() : null;
            var currentMonth = currentMoment ? currentMoment.hMonth() : null;

            this.picker.find('.datepicker-days th:eq(1)')
						.text(moment.localeData('ar').hMonths(moment().hMonth(month)) + ' ' + year);

            var prevMonth = moment(year + ' ' + month + ' 1', 'hYYYY hMM hDD');
            ...
            while (prevMonth.valueOf() < nextMonthVal) {
                ...
                clsName = '';
                //TODO: use diff
                if (prevMonth.hYear() < year || (prevMonth.hYear() == year && prevMonth.hMonth() < month)) {
                    clsName += ' old';
                } else if (prevMonth.hYear() > year || (prevMonth.hYear() == year && prevMonth.hMonth() > month)) {
                    clsName += ' new';
                }
                //TODO: use diff
                ...
                html.push('<td class="day' + clsName + '">' + prevMonth.hDate() + '</td>');
            ...
            if (((this.startDate) && year < this.startDate.hYear()) || ((this.endDate) && year > this.endDate.hYear())) {
                months.addClass('disabled');
            }
            if ((this.startDate) && year == this.startDate.hYear()) {
                months.slice(0, this.startDate.hMonth()).addClass('disabled');
            }
            if ((this.endDate) && year == this.endDate.hYear()) {
                months.slice(this.endDate.hMonth() + 1).addClass('disabled');
            }
            ...
            year -= 1;
            for (var i = -1; i < 11; i++) {
                html += '<span class="year' + (i === -1 || i === 10 ? ' old' : '') + (currentYear === year ? ' active' : '') + (((this.startDate) && year < this.startDate.hYear()) || ((this.endDate) && year > this.endDate.hYear()) ? ' disabled' : '') + '">' + year + '</span>';
                year += 1;
            }
            yearCont.html(html);
        },
                    ...
                    case 'span':
                        if (!target.is('.disabled')) {
                            if (target.is('.month')) {
                                ...
                                this.viewDate.add('hmonths', newMonth - this.viewDate.hMonth());
                    ...
                    case 'td':
                        if (!target.is('.disabled')) {
                            if (target.is('.day')) {
                                ...
                                if (target.is('.old')) {
                                    tempDate.startOf('hmonth').add('days', -1);
                                } else if (target.is('.new')) {
                                    tempDate.endOf('hmonth').add('days', 1);
                                }
                                var month = tempDate.hMonth();
                                var year = tempDate.hYear();
                                this.set(moment(year + ' ' + month + ' ' + day, 'hYYYY hMM hDD'));
